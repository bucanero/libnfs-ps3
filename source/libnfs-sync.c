...
nfs_mount(struct nfs_context *nfs, const char *server, const char *export)
{
	...
	if (nfs_mount_async(nfs, server, export, mount_cb, &cb_data) != 0) {
		nfs_set_error(nfs, "nfs_mount_async failed. %s",
			      nfs_get_error(nfs));
			debugNetPrintf(2,"[LIBNFS] %s %s\n",__FUNCTION__,nfs_get_error(nfs));

		return -1;
	}
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s waiting\n",__FUNCTION__);

	wait_for_nfs_reply(nfs, &cb_data);

	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s end waiting\n",__FUNCTION__);
...
void
callit_cb(struct rpc_context *rpc, int status, void *data _U_ ,void *private_data)
{
