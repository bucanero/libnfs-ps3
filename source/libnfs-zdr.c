...
void libnfs_zdr_destroy(ZDR *zdrs)
{
	...
	if(zdrs)
	{
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s in \n",__FUNCTION__);

		while (zdrs->mem != NULL) {
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s in while \n",__FUNCTION__);

			struct zdr_mem *mem = zdrs->mem->next;
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s in while after next\n",__FUNCTION__);

			//free(zdrs->mem);
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s in while after free\n",__FUNCTION__);

			zdrs->mem = mem;
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s in while after mem \n",__FUNCTION__);
