...
{
	int events;
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s\n",__FUNCTION__);
...
rpc_service(struct rpc_context *rpc, int revents)
{
	...
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s end\n",__FUNCTION__);
...
*/
	...
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s go go\n",__FUNCTION__);
...
//                sin6 = (struct sockaddr_in6 *)&ss;
		...
		if (portOfs == 0) {
			...
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s portOfs=%d portCount=%d\n",__FUNCTION__,portOfs,portCount);
		}
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s \n",__FUNCTION__);

		startOfs = portOfs;
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s startOfs=%d portOfs=%d \n",__FUNCTION__,startOfs,portOfs);
	...
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s port=%d\n",__FUNCTION__,port);

			/* skip well-known ports */
			//if (!getservbyport(port, "tcp")) {

			if (1) {
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s in port=%d portOfs=%d\n",__FUNCTION__,port,portOfs);
...
*/
				...
				rc = bind(rpc->fd, (struct sockaddr *)&ss,
                                          socksize);
				debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s rc=%d \n",__FUNCTION__,rc);

				if(rc==-1)
				{
						debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s jander rc=%d \n",__FUNCTION__,rc);
				...
				{
					debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s wrong chungo rc=%d \n",__FUNCTION__,rc);
	...
	if (connect(rpc->fd, (struct sockaddr *)s, socksize) != 0 &&
            errno != EINPROGRESS) {
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s 1\n",__FUNCTION__);

		rpc_set_error(rpc, "connect() to server failed. %s(%d)",
                              strerror(errno), errno);
					debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s 2\n",__FUNCTION__);
                  ...
                  rpc_cb cb, void *private_data)
{
	...
	rpc->auto_reconnect = 0;
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s before rpc_set_sockaddr\n",__FUNCTION__);

        if (rpc_set_sockaddr(rpc, server, port) != 0) {
                return -1;
        }
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after rpc_set_sockaddr\n",__FUNCTION__);
	...
	if (rpc_connect_sockaddr_async(rpc) != 0) {
		...
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after rpc_connect_sockaddr_async\n",__FUNCTION__);
