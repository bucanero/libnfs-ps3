...
{
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s client_name=%s\n",__FUNCTION__,client_name);
                         ...
                         void *command_data, void *private_data)
{
	...
	if (status != RPC_STATUS_SUCCESS) {
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s no success\n",__FUNCTION__);

		data->cb(rpc, status, command_data, data->private_data);
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after cb\n",__FUNCTION__);

		free_rpc_cb_data(data);
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s end no success\n",__FUNCTION__);

		return;
	}
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s success\n",__FUNCTION__);

        if (rpc_null_async(rpc, data->program, data->version,
                           rpc_connect_program_5_cb, data) != 0) {
  			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after rpc_null_async\n",__FUNCTION__);

                data->cb(rpc, RPC_STATUS_ERROR, command_data, data->private_data);
                		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after cb\n",__FUNCTION__);

                free_rpc_cb_data(data);
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s end after null\n",__FUNCTION__);
                       ...
                       rpc_cb cb, void *private_data)
{
	...
	if (data == NULL) {
			debugNetPrintf(2,"[LIBNFS] %s error malloc\n",__FUNCTION__);
...
nfs_set_error(struct nfs_context *nfs, char *error_string, ...)
{
	...
	debugNetPrintf(2,"[LIBNFS] %s error %s\n",__FUNCTION__, nfs->rpc->error_string);
